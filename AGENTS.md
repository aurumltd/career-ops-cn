...
- **Simplified Chinese (Mainland China market):** `modes/zh/` — native Simplified Chinese translations with China-market vocabulary (社招, 直聘, 五险一金, 年包, 期权, 试用期, 大模型, 算法岗, 平台岗, etc.). Includes `_shared.md`, `评估.md` (evaluation), `申请.md` (apply), `流水线.md` (pipeline), plus China-oriented scan and tracker modes.
...
**When to use Simplified Chinese modes:** If the user is targeting Mainland China roles, asks for Simplified Chinese output, or sets up a China-market search pipeline. Either:
...
3. You detect a Chinese JD and the user is targeting Mainland China → suggest switching to the Chinese setup guide and template files
